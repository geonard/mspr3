...
router.get('/', (req, res) => {
  const faqPath = path.join(__dirname, '../data/security.json');
  fs.readFile(faqPath, 'utf8', (err, data) => {
    if (err) {
      console.error('Erreur lors de la lecture du fichier security:', err);
      res.status(500).json({ error: 'Erreur lors de la récupération de la security' });
      return;
    }
    try {
